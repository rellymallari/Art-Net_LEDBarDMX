...
//Sonoff
#define SONOFF_RELAY_PIN_B 12
#define SONOFF_RELAY_PIN_W1 4
#define SONOFF_RELAY_PIN_W2 14
#define SONOFF_DMX_CHANNEL SONOFF_ADDRESS //(DMX Channels 1-512)
#define SONOFF_DMX_CHANNEL_I SONOFF_DMX_CHANNEL
#define SONOFF_DMX_CHANNEL_W SONOFF_DMX_CHANNEL + 1
#define SONOFF_DMX_CHANNEL_B SONOFF_DMX_CHANNEL + 2
#define SONOFF_STATUS_LED 13
#define SONOFF_MIN_INTENSITY_B 5
#define SONOFF_MIN_INTENSITY_W1 64
#define SONOFF_MIN_INTENSITY_W2 128
...
void setup(void) {
  ...
  //Sonoff
  pinMode(SONOFF_RELAY_PIN_B, OUTPUT);
  digitalWrite(SONOFF_RELAY_PIN_B, LOW);
  pinMode(SONOFF_RELAY_PIN_W1, OUTPUT);
  digitalWrite(SONOFF_RELAY_PIN_W1, LOW);
  pinMode(SONOFF_RELAY_PIN_W2, OUTPUT);
  digitalWrite(SONOFF_RELAY_PIN_W2, LOW);
...
void loop(void){
  ...
  if (millis() - sonoffLastStatusUpdate > 2000) { // No ArtNet signal, port closed
    ...
    if(millis() < 6000){
      //Turn on LEDs if no ArtNet signal on bootup
      digitalWrite(SONOFF_RELAY_PIN_B, HIGH);
      digitalWrite(SONOFF_RELAY_PIN_W1, HIGH);
      digitalWrite(SONOFF_RELAY_PIN_W2, HIGH);
    }
    ...
    if (!digitalRead(SONOFF_ONOFF_BUTTON)){
      if(millis () - sonoffLastButtonPress > 50)
        if(sonoffRelayState) {
          digitalWrite(SONOFF_RELAY_PIN_B, LOW); //off
          digitalWrite(SONOFF_RELAY_PIN_W1, LOW);
          digitalWrite(SONOFF_RELAY_PIN_W2, LOW);
          sonoffRelayState = false;
        }
        else {
          digitalWrite(SONOFF_RELAY_PIN_B, HIGH); //on
          digitalWrite(SONOFF_RELAY_PIN_W1, HIGH);
          digitalWrite(SONOFF_RELAY_PIN_W2, HIGH);
    ...
    if ((millis() % 1000) >= 50){
      if (!digitalRead(SONOFF_STATUS_LED))
  ...
  if (portA[0] == group) {
    if (deviceSettings.portAmode == TYPE_WS2812) {
      ...
      // FX 12 Mode
        ...
        byte* a = artRDM.getDMX(group, port);
        if(a != NULL){
          int16_t dmxChannelWhite = a[SONOFF_DMX_CHANNEL - 1] * a[SONOFF_DMX_CHANNEL_W - 1] / 255; 
          int16_t dmxChannelBlue = a[SONOFF_DMX_CHANNEL - 1] * a[SONOFF_DMX_CHANNEL_B - 1] / 255;

          //W1
          if(dmxChannelWhite >= SONOFF_MIN_INTENSITY_W1){
            digitalWrite(SONOFF_RELAY_PIN_W1, HIGH);  //on
          }
          else {
            digitalWrite(SONOFF_RELAY_PIN_W1, LOW);  //off
          } 
          //W2
          if(dmxChannelWhite >= SONOFF_MIN_INTENSITY_W2){
            digitalWrite(SONOFF_RELAY_PIN_W2, HIGH);  //on
          }
          else {
            digitalWrite(SONOFF_RELAY_PIN_W2, LOW);  //off
          }            
          //B
          if(dmxChannelBlue >= SONOFF_MIN_INTENSITY_B){
            digitalWrite(SONOFF_RELAY_PIN_B, HIGH);  //on
          }
          else {
            digitalWrite(SONOFF_RELAY_PIN_B, LOW);  //off
